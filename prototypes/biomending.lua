function generate_biomending_recipe_icons_from_item(item)
  ...
  if item.icons == nil then
    icons = {
      {icon = "__wood-universe-assets__/graphics/icons/biomending.png"},
      {
        icon = item.icon,
        icon_size = item.icon_size,
        scale = (0.5 * defines.default_icon_size / (item.icon_size or defines.default_icon_size)) * 0.8,
      },
      {icon = "__wood-universe-assets__/graphics/icons/biomending-top.png"}
    }
  else
    icons = {{icon = "__wood-universe-assets__/graphics/icons/biomending.png"}}
    ...
    icons[#icons + 1] = {icon = "__wood-universe-assets__/graphics/icons/biomending-top.png"}
  end
  return icons
end
  ...
  data:extend({
    {
      ...
      localised_name = {item.localised_name or ("item-name."..entry.name), {"recipe-name.biomending"}},
      ...
      ingredients = {{type="item", name=entry.name, amount=1, ignored_by_stats=1, ignored_by_productivity=1}},
      results = {{type="item", name=entry.name, amount=1, probability=0.99, percent_spoiled=0.75, ignored_by_stats=1, ignored_by_productivity=1}},
